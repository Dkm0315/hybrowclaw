...
const steps: readonly Step[] = [
  ...
  },
  {
    ...
    choices: [
      ...
      choice("whatsapp", "WhatsApp", "Business phone ID, access token, verify token, and webhook secret.", "business", "lime", "WhatsApp should default to human-reviewed drafts for customer-facing messages.", controls("Reply mode", "Draft first", "Visibility", "Selected numbers"), channelCommands("whatsapp")),
...
function channelCommands(id: string): readonly string[] {
  const commands: Record<string, readonly string[]> = {
    ...
    whatsapp: ["muster channels setup whatsapp --access-token-env WHATSAPP_ACCESS_TOKEN --verify-token-env WHATSAPP_VERIFY_TOKEN --phone-number-id-env WHATSAPP_PHONE_NUMBER_ID --public-url https://your-domain.example"],
